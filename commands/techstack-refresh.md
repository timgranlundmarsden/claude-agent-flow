...
"Run the Technology Discovery Protocol for this project. TECHSTACK.md is [missing / stale — last scanned: <datetime>]. Follow the matching case (A or C) exactly and return a `TECHSTACK DISCOVERY:` section with the required output. If TECHSTACK.md is missing, return either A1 (full proposed content) or A2 (greenfield note) — never skip."
...
**If TECHSTACK.md is stale:** Invoke the author agent: "Edit `TECHSTACK.md` at the project root — apply only the following accepted changes: <list accepted changes>. Do not modify any other content."
