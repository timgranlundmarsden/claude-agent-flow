...
2. Use WebSearch to find current information; use WebFetch to read full content from specific URLs. If `AGENT_FLOW_WEB_SEARCH_ENABLED` is set to `true` or `1`, prefer `.claude/skills/web-search/web-search.sh` for real-time grounded search; otherwise use native WebSearch.
...
Your output is information — the calling context decides what to do with it.

If research reveals a significantly better approach, flag it clearly with APPROACH CHANGE RECOMMENDED before the findings.

Output length: as many findings blocks as the question requires. No preamble.
