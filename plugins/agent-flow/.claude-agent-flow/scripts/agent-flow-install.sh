...
#   agent-flow-install.sh --scope plugin --plugin-dir /path/to/plugins/agent-flow
#
#   # Update mode (re-sync from source):
#   agent-flow-install.sh --update --scope plugin+github --plugin-dir /path/to/plugins/agent-flow
...
if [[ "$SCOPE" != "sandbox" ]]; then
  ...
  if [[ -f "$SETTINGS_FILE" ]]; then
    ...
    jq --arg mid "$MARKETPLACE_ID" --arg repo "$SOURCE_REPO" --arg pname "$AF_PLUGIN_NAME" '
      ...
      .enabledPlugins //= {} |
      .extraKnownMarketplaces[$mid] = {"source": {"source": "github", "repo": $repo}} |
...
if [[ "$SCOPE" == "plugin+github" || "$SCOPE" == "sandbox" ]]; then
  ...
  [[ -z "$WF_SOURCE_REL" ]] && WF_SOURCE_REL=".claude-agent-flow/workflows/"
  ...
  if [[ -d "$WF_SOURCE" ]]; then
    ...
    if [[ "$SCOPE" == "plugin+github" ]]; then
      ...
      # Sandbox mode: copy all workflows
      rsync -a --exclude='.DS_Store' "${WF_SOURCE%/}/" "${TARGET_DIR%/}/${WF_TARGET_REL%/}/"
      echo "  [copy] $WF_SOURCE_REL -> $WF_TARGET_REL (all)"
    fi
  else
