...
- **`@plans/filename.md`** — Read the file. Scan for a line matching `## Backlog Task: TASK-XX`. Extract the task ID. If no such line exists, treat as "plan file without task link" (see below).
- **`TASK-XX`** — Run `backlog task <id> --plain` via Bash to load the full task context (description, acceptance criteria, plan, notes). Use this as the implementation brief for the rest of the pipeline.
- **`@backlog/tasks/task-N - Title.md`** — Read the file. Extract the task ID from the `id:` field in file content if present (authoritative). Otherwise derive from the filename: the numeric part before the first space is the task ID with no padding (e.g. `task-37 - Backlog.md-Agent-Flow-Integration.md` → `TASK-37`).
...
All tracking uses `backlog` CLI via Bash. Double-quoted strings only. Hooks auto-push after each operation.
...
24a. If `task_id` is set and the current branch is NOT main/master:
    ...
    3. If no PR exists, create one:
       ...
       ```
    4. Store the PR URL
    5. Update task notes: `backlog task edit <task_id> --append-notes "PR created: <pr_url>"`
    6. If GitHub MCP tools are unavailable or ToolSearch fails, skip PR creation with a warning — do not block the pipeline

### Phase 6 — Report
25. **Only after Phase 5b (commit+push) and Phase 5c (PR creation) are fully complete:**
    If `task_id` is set:
    ```
    ...
    successfully — including the final commit, push, PR creation, and all agent work.
...
from `--loops N`): STOP.
Set task to Blocked: `backlog task edit <task_id> -s "Blocked" --append-notes "Blocked: max iterations reached - <remaining issues>"`.
...
`backlog task edit <task_id> -s "Blocked" --append-notes "Blocked: <reason>"`.
